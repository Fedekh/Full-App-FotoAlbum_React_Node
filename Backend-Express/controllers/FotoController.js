...
async function show(req, res, next) {
  try {
    ...
    console.error("Errore durante il recupero della foto:", error);
    return res
      .status(500)
...
async function store(req, res, next) {
  ...
  if (!validation.isEmpty()) {
    ...
    return res
      .status(422)
      .json({
        error: "Controllare i dati inseriti",
  ...
  try {
    ...
    if (categories) {
      const categoryIds = categories
        .split(",")
        .map((idCategory) => +idCategory.trim());
    ...
    return res
      .status(500)
      .json({
        error: "Errore durante la creazione della foto",
...
async function update(req, res, next) {
  try {
    ...
    return res.json({
      message: `Aggiornamento di ${data.name} completato con successo`,
    });
  } catch (error) {
...
async function destroy(req, res, next) {
  try {
    ...
    if (data.userId === req.user.id) {
      ...
      return res.json({
        message: `${data.name} è stato eliminato con successo`,
      });
    } else {
      const authError = new AuthError("Accesso non autorizzato", 403);
      res.status(authError.status).json(authError.resp());
